...
- **词库更新** — 设置页内置词库在线更新：从 Danbooru 同步新标签与热度计数，快速 / 完整两档可选，实时进度与结果统计；新标签按 Danbooru 分类自动归位——角色 →「作品角色」、版权 →「作品-文化-引用」、元数据 →「文字-图形-符号-界面」、画师 → 画师库，仅通用标签归入「未分类(在线新增)」；历史更新产生的「未分类」角色标签会在下次更新时自动迁移归位
